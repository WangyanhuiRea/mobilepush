...
NSString *const PUSH_TYPE_MESSAGE = @"message";
NSString *const PUSH_TYPE_NOTIFICATION = @"notification";



@interface RNMobilepush () <UNUserNotificationCenterDelegate>
@end

@implementation RNMobilepush
{
    bool hasListeners;
}


static RNMobilepush * sharedInstance = nil;
...
RCT_EXPORT_MODULE(RNMobilepush);
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions options))completionHandler
{
    ...
    notificationDict[@"type"] = PUSH_TYPE_NOTIFICATION;
...
- (void)onMessageReceived:(NSNotification *)notification
{
    ...
    // 类型 “notification” or "message"
    notificationDict[@"type"] = PUSH_TYPE_MESSAGE;
...
- (void)sendEventToJs:(NSMutableDictionary*)notification
{
    ...
    //修正app退出后，点击通知会闪退bug
    RNMobilepush* __weak weakSelf = self;
    if(!hasListeners){
